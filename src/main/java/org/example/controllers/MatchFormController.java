...
public class MatchFormController {
    ...
    private void setupValidationListeners() {
        ...
        cbTournoi.valueProperty().addListener((obs, old, val) -> {
            ...
            // If tournament changes, re-validate date if it exists
    ...
    private boolean isInputValid() {
        ...
        if (sel == null) {
            ...
            // Force DatePicker to sync its value from editor text before getting value
            if (dpDate.getEditor().getText() != null && !dpDate.getEditor().getText().isEmpty()) {
                try {
    ...
    void onSave(ActionEvent event) {
        ...
        try {
            if (existingMatch == null) {
                Match m = new Match(
                    txtEquipe1.getText(), txtEquipe2.getText(),
                    Integer.parseInt(txtScore.getText()), dpDate.getValue(),
                    txtPrix.getText(), cbTournoi.getValue().getId(),
                    txtMode.getText(), txtLocalisation.getText()
                );
                matchService.add(m);
            } else {
                Match m = new Match(
