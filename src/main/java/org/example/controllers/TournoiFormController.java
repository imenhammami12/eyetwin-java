...
import org.example.entities.TypeTournoi;
...
public class TournoiFormController {
    ...
    public void setOnSaved(Runnable callback) { this.onSavedCallback = callback; }

    // ─── Validation ────────────────────────────────────────────────────────────

    private void setError(Label lbl, String msg) {
        if (msg == null || msg.isEmpty()) { lbl.setText(""); lbl.setVisible(false); lbl.setManaged(false); }
        else { lbl.setText(msg); lbl.setVisible(true); lbl.setManaged(true); }
    }

    private void clearErrors() {
        setError(errNom, null); setError(errDateDebut, null); setError(errDateFin, null);
        setError(errDescription, null); setError(errImage, null); setError(errType, null); setError(errPrix, null);
    }

    private boolean isValid() {
        clearErrors(); boolean ok = true;
        if (txtNom.getText().trim().isEmpty())         { setError(errNom, "Nom obligatoire."); ok = false; }
        LocalDate now = LocalDate.now();
        LocalDate debut = dpDateDebut.getValue(), fin = dpDateFin.getValue();
        if (debut == null)                             { setError(errDateDebut, "Date de début requise."); ok = false; }
        else if (debut.isBefore(now))                 { setError(errDateDebut, "Doit être >= aujourd'hui."); ok = false; }
        if (fin == null)                               { setError(errDateFin, "Date de fin requise."); ok = false; }
        else if (debut != null && fin.isBefore(debut)) { setError(errDateFin, "Doit être >= début."); ok = false; }
        if (txtDescription.getText().trim().isEmpty()) { setError(errDescription, "Description obligatoire."); ok = false; }
        if (txtImage.getText().trim().isEmpty())       { setError(errImage, "Image obligatoire."); ok = false; }
        if (cbType.getValue() == null)                 { setError(errType, "Type obligatoire."); ok = false; }
        if (txtPrix.getText().trim().isEmpty()) { setError(errPrix, "Prix obligatoire."); ok = false; }
        else { try { if (Double.parseDouble(txtPrix.getText()) <= 0) { setError(errPrix, "Doit être > 0."); ok = false; } }
               catch (NumberFormatException e) { setError(errPrix, "Nombre valide requis."); ok = false; } }
        return ok;
    }
    ...
    @FXML void onBrowseImage(ActionEvent event) {
        ...
        if (f != null) txtImage.setText(f.getAbsolutePath());
    }
    ...
    @FXML void onCancel(ActionEvent event) { closeWindow(); }
