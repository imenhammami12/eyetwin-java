...
public class ReviewServiceImpl implements IReviewService {
    ...
    public Review createReview(Review review) throws SQLException {
        if (review.getSentiment() == null) {
            try {
                review.setSentiment(
                        sentimentService.analyze(review.getContent(), review.getRating()).join());
            } catch (Exception e) {
    ...
    public Review getReviewById(int id) throws SQLException {
        ...
        if (rs.next()) return mapReviewWithUser(rs);
        return null;
    }
    ...
    public List<Review> getAllReviews() throws SQLException {
        ...
        PreparedStatement ps = c.prepareStatement(sql);
        ResultSet rs = ps.executeQuery();
        while (rs.next()) reviews.add(mapReviewWithUser(rs));
        return reviews;
    }
    ...
    public List<Review> getAllReviewsWithPlanning() throws SQLException {
        String sql = """
            SELECT r.*,
                   ...
                   p.IDplanning AS p_id, p.type AS p_type, p.level AS p_level,
                   p.date AS p_date, p.time AS p_time
            FROM review r
            INNER JOIN user u ON u.id = r.user_id
    ...
    public void updateSentiment(int reviewId, String sentiment) throws SQLException {
        Connection c = getConnection();
        PreparedStatement ps = c.prepareStatement(
                "UPDATE review SET sentiment = ? WHERE id = ?");
    ...
    public boolean hasUserReviewedPlanning(int userId, int idPlanning) throws SQLException {
        Connection c = getConnection();
        PreparedStatement ps = c.prepareStatement(
                "SELECT id FROM review WHERE user_id = ? AND ID_planning = ?");
        ps.setInt(1, userId);
    ...
    public int countReviews(int idPlanning) throws SQLException {
        Connection c = getConnection();
        PreparedStatement ps = c.prepareStatement(
                "SELECT COUNT(*) FROM review WHERE ID_planning = ?");
        ps.setInt(1, idPlanning);
        ResultSet rs = ps.executeQuery();
