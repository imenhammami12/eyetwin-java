...
public class CoachApplicationServiceImpl implements ICoachApplicationService {
    ...
    public boolean hasPendingApplication(int userId) throws SQLException {
        ...
        PreparedStatement ps = conn.prepareStatement(
                "SELECT COUNT(*) FROM coach_application WHERE user_id = ? AND status = 'PENDING'");
        ps.setInt(1, userId);
        ResultSet rs = ps.executeQuery();
    ...
    public CoachApplication findLatestByUserId(int userId) throws SQLException {
        Connection conn = getConnection();
        PreparedStatement ps = conn.prepareStatement(
                "SELECT * FROM coach_application WHERE user_id = ? ORDER BY submitted_at DESC LIMIT 1");
        ps.setInt(1, userId);
        ResultSet rs = ps.executeQuery();
    ...
    public void approve(int applicationId, String comment, int adminUserId) {
        ...
        try {
            ...
            if (currentRoles == null || !currentRoles.contains("ROLE_COACH")) {
                ...
                ps2.setString(1, newRoles);
                ps2.setInt(2, app.getUserId());
                ps2.executeUpdate();
            }

            con.commit();
            System.out.println("[CoachApplicationService] ✅ Approved #" + applicationId);
        } catch (Exception e) {
    ...
    public void reject(int applicationId, String comment, int adminUserId) {
        ...
        try {
            ...
            ps.executeUpdate();
            System.out.println("[CoachApplicationService] ✅ Rejected #" + applicationId);
        } catch (Exception e) {
    ...
    public Map<String, Integer> getGlobalStats() {
        ...
        try {
            Connection con = getConnection();
            PreparedStatement ps = con.prepareStatement(
                    "SELECT status, COUNT(*) AS cnt FROM coach_application GROUP BY status");
            ResultSet rs = ps.executeQuery();
            int total = 0;
            while (rs.next()) {
